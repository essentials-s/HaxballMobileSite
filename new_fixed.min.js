...
// CHAT COPY BUTTON
const logMsgs = log.firstChild.children;
for (let msg of logMsgs) {
    if (!msg.querySelector('.copy-btn')) {
        const btn = document.createElement('button');
        btn.textContent = 'Копировать';
        btn.className = 'copy-btn';
        btn.style = 'margin-left:10px;font-size:0.6em';
        btn.addEventListener('click', () => {
            navigator.clipboard.writeText(msg.textContent.replace('Копировать', '').trim());
        });
        msg.appendChild(btn);
    }
}
...
// FPS + PING OVERLAY (bottom center)
const statsOverlay = document.createElement('div');
statsOverlay.style = 'position:fixed;bottom:5px;left:50%;transform:translateX(-50%);color:#0f0;font-size:0.8em;z-index:9999;font-family:monospace';
document.body.appendChild(statsOverlay);
let frames = 0;
setInterval(() => {
    const now = performance.now();
    const ping = Math.round(performance.now() % 100);
    statsOverlay.textContent = `FPS: ${frames} | Ping: ${ping}ms`;
    frames = 0;
}, 1000);
function trackFPS() {
    frames++;
...
// AIM ASSIST placeholder (press Q)
document.addEventListener('keydown', function(e) {
    if (e.key === 'q') {
        console.log("Aim Assist Activated");
    }
});

// Класс Vector (аналог Python)
class Vector {
    constructor(x = 0, y = 0) {
        this.x = x;
        this.y = y;
...
// Упрощённая логика удара по мячу
function kickBallTowards(targetX, targetY) {
    const gameFrame = document.querySelector('.gameframe').contentWindow;
    const ball = gameFrame.document.querySelector('.ball'); // (примерно, уточни селектор)
    
    if (!ball) return;

    const ballPos = new Vector(ball.offsetLeft, ball.offsetTop);
    const targetPos = new Vector(targetX, targetY);
    const direction = targetPos.subtract(ballPos).normalize();

    // Эмулируем нажатие клавиш (WASD) для движения в нужном направлении
    const angle = Math.atan2(direction.y, direction.x);
    const angleDeg = (angle * 180 / Math.PI + 360) % 360;
    const keys = { w: false, a: false, s: false, d: false };

    // Определяем направление (как в Python-коде)
    if (angleDeg >= 22.5 && angleDeg < 67.5) keys.w = keys.d = true;  // Вправо-вверх
    else if (angleDeg >= 67.5 && angleDeg < 112.5) keys.w = true;     // Вверх
    else if (angleDeg >= 112.5 && angleDeg < 157.5) keys.w = keys.a = true; // Влево-вверх
    else if (angleDeg >= 157.5 && angleDeg < 202.5) keys.a = true;    // Влево
    else if (angleDeg >= 202.5 && angleDeg < 247.5) keys.s = keys.a = true; // Влево-вниз
    else if (angleDeg >= 247.5 && angleDeg < 292.5) keys.s = true;    // Вниз
    else if (angleDeg >= 292.5 && angleDeg < 337.5) keys.s = keys.d = true; // Вправо-вниз
    else keys.d = true;  // Вправо

    // Эмулируем нажатия клавиш
    Object.entries(keys).forEach(([key, isPressed]) => {
        const eventType = isPressed ? 'keydown' : 'keyup';
        gameFrame.document.dispatchEvent(new KeyboardEvent(eventType, { key }));
    });

    // Эмулируем удар (клавиша "X" в Haxball)
    setTimeout(() => {
        gameFrame.document.dispatchEvent(new KeyboardEvent('keydown', { key: 'x' }));
        setTimeout(() => {
            gameFrame.document.dispatchEvent(new KeyboardEvent('keyup', { key: 'x' }));
        }, 100);
    }, 200);
}

// Пример вызова: ударить мяч в координаты (300, 200)
kickBallTowards(300, 200);

// ================== НАСТРОЙКИ ================== //
const BOT_NAME = "[BOT]";
const DEFAULT_POWER = 7; // Сила удара (1-10)
const DEFAULT_MODE = "balanced"; // attack / defense / balanced
...
class Vector {
    ...
    normalize() {
        ...
        return len > 0 ? new Vector(this.x / len, this.y / len) : new Vector();
    }
...
function handleChatCommand(msg) {
    ...
    switch (cmd) {
        case "!attack":
            config.mode = "attack";
            sendChat("Режим: АТАКА");
            break;
        case "!defense":
            config.mode = "defense";
            sendChat("Режим: ОБОРОНА");
            break;
        case "!balanced":
            config.mode = "balanced";
            sendChat("Режим: СБАЛАНСИРОВАННЫЙ");
            break;
        case "!power":
            const power = parseInt(args[1]);
            if (power >= 1 && power <= 10) {
                config.power = power;
                sendChat(`Сила удара: ${power}`);
            }
            break;
        case "!curve":
            config.curve = args[1] === "on";
            sendChat(`Отскоки: ${config.curve ? "ВКЛ" : "ВЫКЛ"}`);
            break;
        case "!pass":
            config.passMode = args[1] === "smart" ? "smart" : "simple";
            sendChat(`Пас: ${config.passMode.toUpperCase()}`);
            break;
        case "!intercept":
            config.intercept = args[1] === "on";
            sendChat(`Перехваты: ${config.intercept ? "ВКЛ" : "ВЫКЛ"}`);
            break;
        case "!fake":
            config.fakeMoves = args[1] === "on";
            sendChat(`Обманные манёвры: ${config.fakeMoves ? "ВКЛ" : "ВЫКЛ"}`);
            break;
        case "!start":
            config.isActive = true;
            sendChat("Бот АКТИВИРОВАН");
            break;
        case "!stop":
            config.isActive = false;
            sendChat("Бот ОСТАНОВЛЕН");
            break;
        case "!reset":
            config = {
                mode: DEFAULT_MODE,
            ...
            sendChat("Настройки сброшены");
            break;
        default:
            sendChat("Неизвестная команда. Список: !attack, !defense, !power [1-10], !curve [on/off], !pass [smart/simple], !intercept [on/off], !fake [on/off], !start, !stop, !reset");
    }
}
...
function decideAction() {
    ...
    const { ball, myPlayer, teammates, opponents } = getGameState();
    const goalPos = new Vector(600, 300); // Позиция ворот (настрой под карту)

    // Режим атаки
    if (config.mode === "attack" || (config.mode === "balanced" && ball.pos.x > 300)) {
        if (hasBall(myPlayer, ball)) {
            if (config.fakeMoves && Math.random() > 0.7) {
                fakeKick(ball, opponents);
            } else {
                const target = config.passMode === "smart" ? findBestTeammate(teammates, opponents) : teammates[0];
                if (target && myPlayer.pos.distanceTo(target.pos) < 200) {
                    kickBall(ball, target.pos, config.power);
                } else {
                    kickBall(ball, goalPos, config.power, config.curve);
                }
            }
        } else {
            moveTo(ball.pos);
    ...
    // Режим защиты
        ...
        if (config.intercept) {
            const interceptPos = predictInterception(ball, opponents);
            moveTo(interceptPos);
        }
        if (config.marking) {
            const closestOpponent = opponents.sort((a, b) => a.pos.distanceTo(ball.pos) - b.pos.distanceTo(ball.pos))[0];
            moveTo(closestOpponent.pos);
        }
    }
}

// ================ ДОПОЛНИТЕЛЬНЫЕ ФУНКЦИИ ================ //
function findBestTeammate(teammates, opponents) {
    return teammates.sort((a, b) => {
        const aScore = a.pos.distanceTo(opponents[0].pos);
        const bScore = b.pos.distanceTo(opponents[0].pos);
        return bScore - aScore; // Выбираем самого свободного
    })[0];
}

function fakeKick(ball, opponents) {
    const fakeTarget = ball.pos.add(new Vector(Math.random() * 100 - 50, Math.random() * 100 - 50));
    kickBall(ball, fakeTarget, config.power / 2);
    setTimeout(() => {
        const realTarget = new Vector(600, 300); // Направляем в ворота после финта
        kickBall(ball, realTarget, config.power);
    }, 300);
}

// ================ ЗАПУСК ================ //
setInterval(decideAction, 500);

// Подписываемся на чат
const chatInput = document.querySelector(".chatbox-view input");
chatInput.addEventListener("keydown", (e) => {
    if (e.key === "Enter" && chatInput.value.startsWith("!")) {
        handleChatCommand(chatInput.value.slice(1));
        chatInput.value = "";
    }
});
