...
function showControls(v) {
    ...
    if (window.showChatButtons) window.showChatButtons(v);} else {
...
function createFPSCounter() {
    ...
    function updateFPS() {
        if (show) {
            fps.textContent = 'FPS: ' + (Math.floor(Math.random() * (1000 - 240 + 1)) + 240) + ' | Erdaulet Team';
        }
    }
    setInterval(updateFPS, 1000);
}

// === 2/10: BALL DIRECTION ARROW ===
...
// === 3/10: BIND CHAT BUTTONS ===
function setupChatButtons() {
    const ufBtn = document.createElement("button");
    ufBtn.setAttribute("class", "neo rounded");
    ufBtn.setAttribute("float", "");
    ufBtn.setAttribute("view", "hidden");
    ufBtn.style.cssText = "position: fixed; bottom: 17vw; right: 5%; width: 14vw; height: 14vw; font-size: 0.7rem; z-index: 100;";
    ufBtn.innerText = "уф";
    ufBtn.addEventListener("click", () => prefabMessage("уф"));
    document.body.appendChild(ufBtn);

    const passBtn = document.createElement("button");
    passBtn.setAttribute("class", "neo rounded");
    passBtn.setAttribute("float", "");
    passBtn.setAttribute("view", "hidden");
    passBtn.style.cssText = "position: fixed; bottom: 31vw; right: 5%; width: 14vw; height: 14vw; font-size: 0.7rem; z-index: 100;";
    passBtn.innerText = "дай пас";
    passBtn.addEventListener("click", () => prefabMessage("дай пас"));
    document.body.appendChild(passBtn);

    window.showChatButtons = function (v) {
        ufBtn.setAttribute("view", v ? "visible" : "hidden");
        passBtn.setAttribute("view", v ? "visible" : "hidden");
    };
}
...
function setupRadar() {
    ...
    function renderRadar() {
        const frame = document.querySelector('.gameframe');
        if (!frame) return requestAnimationFrame(renderRadar);

        const doc = frame.contentWindow?.document;
        if (!doc) return requestAnimationFrame(renderRadar);

        const players = doc.querySelectorAll('[data-hook^="player-"]');
        const ball = doc.querySelector('[data-hook="ball"]');
        ctx.clearRect(0, 0, radar.width, radar.height);

        function drawDot(el, color) {
            const rect = el.getBoundingClientRect();
            const x = (rect.left / window.innerWidth) * radar.width;
            const y = (rect.top / window.innerHeight) * radar.height;
            ctx.fillStyle = color;
            ctx.beginPath();
        ...
        players.forEach(p => drawDot(p, "#00f")); // синие игроки
        if (ball) drawDot(ball, "#ff0"); // мяч
...
function setupPingDisplay() {
    ...
    setInterval(() => {
        const pings = performance.now() % 100; // фейковый пинг
        pingDiv.textContent = 'Ping: ' + (50 + Math.floor(pings)).toString() + ' ms';
    }, 1000);
}
...
function setupThemeSwitcher() {
    ...
    toggleBtn.onclick = () => {
        ...
        style.innerHTML = dark
            ? 'body{background:#1a2125 !important;color:white;}'
...
// === 7/10: AUTO-SPECTATE ===
function setupAFKHandler() {
    let lastMove = Date.now();
    ['mousemove', 'touchstart', 'keydown'].forEach(evt =>
        window.addEventListener(evt, () => (lastMove = Date.now()))
    );

    setInterval(() => {
        const now = Date.now();
        if (now - lastMove > 30000) {
            const frame = document.querySelector('.gameframe');
            const leaveBtn = frame?.contentWindow?.document?.querySelector('[data-hook="spec-btn"]');
...
// === 8/10: BASIC STATS TRACKER ===
function setupStatsTracker() {
    let goals = 0, passes = 0;
    ...
    setInterval(() => {
        statBox.textContent = 'Goals: ' + goals + ' | Passes: ' + passes;
    }, 1000);

    const observer = new MutationObserver(() => {
        const log = document.querySelector('.gameframe')?.contentWindow?.document?.querySelector('[data-hook="log"]');
        if (!log) return;
        const last = log.textContent.toLowerCase();
        if (last.includes('goal')) goals++;
        if (last.includes('pass')) passes++;
    });
    const logBox = document.querySelector('.gameframe')?.contentWindow?.document?.querySelector('[data-hook="log"]');
    if (logBox) observer.observe(logBox, { childList: true, subtree: true });
}



// === 9/10: RANDOM ROOM BUTTON ===
function setupRandomRoomButton() {
    const btn = document.createElement('button');
    btn.textContent = 'Random Room';
    btn.style.cssText = \`
        position: fixed;
        bottom: 5px;
        left: 10px;
    ...
    \`;
    btn.onclick = () => {
        const rows = document.querySelector('.gameframe')?.contentWindow?.document?.querySelectorAll('tr');
        const list = Array.from(rows).filter(r => r.style.display !== 'none');
        if (list.length > 0) {
            const randomRow = list[Math.floor(Math.random() * list.length)];
            randomRow.click();
        }
    };
