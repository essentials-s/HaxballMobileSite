...
(function() {
    ...
    fpsMeter.style.cssText = 'position:fixed;top:10px;left:10px;background:#222;padding:5px 10px;border-radius:8px;color:#9f7aea;font-family:sans-serif;z-index:9999;font-size:14px;opacity:0.8;';
    ...
    function updateFPS() {
        ...
        if (now - lastFrameTime >= 1000) {
            fpsMeter.innerText = `FPS: ${frames} | PING: ${lastPing}ms`;
            frames = 0;
            lastFrameTime = now;
        }
    ...
    setInterval(() => {
        if (window?.gameFrame?.connection) {
            lastPing = Math.round(window.gameFrame.connection.ping);
        }
    }, 1000);
})();
...
(function() {
    ...
    modMenu.id = 'customModMenu';
    modMenu.innerHTML = \`
    <button onclick="alert('Auto Kick ON')">AutoKick</button>
    <button onclick="alert('Fake Ping Activated')">Fake Ping</button>
    <button onclick="alert('Minimap Toggle')">Minimap</button>
    <button onclick="alert('Predictor ON')">Ball Predictor</button>
    <button onclick="alert('Spam Started')">Chat Spam</button>
    \`;
